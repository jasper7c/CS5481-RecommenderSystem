...
class NGCNRecommender(BaseRecommender, nn.Module):
    ...
    def _build_adjacency_matrix(self, data):
        """改进后的邻接矩阵构建（去重+向量化计算）"""
        # 获取去重后的交互数据
        unique_data = data[['userId', 'movieId']].drop_duplicates()
        user_indices = unique_data['userId'].map(self.user_mappings['id_to_idx']).tolist()
        item_indices = unique_data['movieId'].map(self.item_mappings['id_to_idx']).tolist()
    ...
    def _train_model(self, data):
        ...
        item_indices = data['movieId'].map(self.item_mappings['id_to_idx']).tolist()
    ...
    def _init_mappings(self, data):
        ...
        self.item_mappings = {
            'idx_to_id': dict(enumerate(data['movieId'].unique())),
            'id_to_idx': {id: idx for idx, id in enumerate(data['movieId'].unique())}
        }